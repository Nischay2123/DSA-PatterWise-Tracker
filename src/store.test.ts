...
describe("the API key never leaves the browser in an export", () => {
  ...
  it("keeps provider and model, which are not secrets", () => {
    const v2 = v2Reducer(emptyAppStoreV2(), { type: "SET_SETTINGS", patch: { apiKey: "k", provider: "groq", model: "openai/gpt-oss-120b" } });
    const exported = toExportableV2(v2);
    expect(exported.settings.provider).toBe("groq");
    expect(exported.settings.model).toBe("openai/gpt-oss-120b");
  });
});
